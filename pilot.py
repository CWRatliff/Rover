...
latmet = 30.86                  	    # meters per second of latitude
lonmet = -30.92 * math.cos(latitude)
...
routes = [[0,0],                    #0
    ...
    [11, 12, 14, 17, 18, 19, 20, 21, 22, 24, 25, 26, 27, 0],        #3 - E.F. meander
...
# cvt deg lat/lon to Z11N UTM
def vsec2met(latsec, lonsec):
    vlat = latsec / 3600 + latbias
    vlon = lonsec / 3600 + lonbias
    yZ11, xZ11 = Z11Nxy(vlat, vlon)
    mxdel = xZ11 - biasZ11x
    mydel = yZ11 - biasZ11y
    return(mydel, mxdel)
...
def route_waypoint(schr):
    ...
    try:
        ...
        if wpt == 0:                # end of waypoint / route
            ...
            if wpt == 1:                    # <<<<<<< RTB >>>>>>>>>
                route = bestroute(posAV, waypts[11])
            ...
            elif (wpt >= 10 and wpt <= 62): #start of waypoint
...
#            posAV = vsec2ft(ilatsec, ilonsec)
            posAV = vsec2met(ilatsec, ilonsec)
            if (not wptflag):
                cgstr = "{lt%5.1f} " % posAV[0]
                sendit(cgstr)
                logit(cgstr)
                cgstr = "{ln%5.1f} " % posAV[1]
                sendit(cgstr)
                logit(cgstr)
            gpsEpoch = time.time()
            gpsokflag = True
