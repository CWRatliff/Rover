...
# controller commands
            if (xchr >= 'A') and (xchr <= 'Z'):
                if (xchr == 'D'):       # single digit keypad commands
                    xchr = cbuff[2]
                    simple_commands(xchr)
                    cogBase = 0         #invalidate COG baseline

                elif xchr == 'E':       # Keypad commands preceded by a star
                    xchr = cbuff[2]
                    star_commands(xchr)
                    cogBase = 0         #invalidate COG baseline

                elif xchr == 'F':       # Keypad commands preceeded by a #
