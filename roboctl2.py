# N.B. lin 67 port name system dependent
# line 59 is 32 bit/64 bit OS dependent
# line 976 changes with database updates
# 211208 - goto spot work
# 230730 - switches added to pantilt
# 240415 - many small changes to mollify Assistant warnings
#			tkinter create_text generates 3 lines of complaint per line
#			of code
#			mouse motion still very slow
# 260616 - gpiozio substituted for RPi.GPIO
#          units in meters, UMS Zone 11N positional basis
# 260713 - Xspot switched from using usf to utm

# on change from Pi5 to Pi3:
#   Pi5:ser = serial.Serial(port='/dev/ttyAMA0',      #xbee to rover
#   Pi3:ser = serial.Serial(port='/dev/ttyS0',      #xbee to rover

#   Pi5:"/home/pi/libsapphire.so"
#   Pi3:"/home/pi/libsapphire32.so"

# update database filename

BUTTON = 1      # 0 - gpiozero, 1 - RPi.GPIO
...
stlon = 300
scale = 3.0
biasZ11y = 3789959.718	# 34-14 N
biasZ11x = 309655.965	# -119-04 W
...
if BUTTON == 0:
    buttongreen = gz.Button(GREEN, bounce_time = 0.1)
    buttonblack = gz.Button(BLACK, bounce_time = 0.1)
    buttonred = gz.Button(RED, bounce_time = 0.1)
    buttonblue = gz.Button(BLUE, bounce_time = 0.1)
    buttonyellow = gz.Button(YELLOW, bounce_time = 0.1)
else:
...
cdlib = ctypes.CDLL("/home/pi/libsapphire32.so")
...
# Serial I/O must be enabled at system level
...
root.wm_title('Rover Controller 260611')
...
trtype=Label(tname,text="Type:", font=("bold",18),
    textvariable=treetype).grid(row=1,column=0)
...
#================== Button ISR's ==================================
# single tap - right 1 deg.
# double tap - right 5 deg.
# triple tap - right 35 deg. or right pivot
# if BUTTON == 1, these ISR's will never be called
def Button_green():
    global lastpress
    global butngreen2
    global greenepoch
    if lastpress == GREEN:
        if ((time.time() - greenepoch) < bb_delay): # if less than .6 sec
            if butngreen2:                 # if double tap in progress
                # print("green - 35 deg rightturn") # 35 deg turn
                dxmit('9')
                butngreen2 = False	# reset 
                # print("green - 5 degree")  # 5 deg steering
                dxmit('6')
                butngreen2 = True   # double tap going
                greenepoch = time.time()    # restart timer
            return
    # print("green - 1 deg")
    dxmit('3')
    butngreen2 = False          # double tap started
    lastpress = GREEN
...
def Button_red():
    ...
    global redepoch
    if lastpress == RED:
        if ((time.time() - redepoch) < bb_delay): # if less than .6 sec
            if butnred2:    	# if double tap in progress
                # print("red - 35 deg leftturn") # 35 deg turn
                dxmit('7')
                butnred2 = False
                lastpress = None
            else:
                # print("red - 5 degree")  # 5 deg steering
                dxmit('4')
                butnred2 = True # double tap started
                redepoch = time.time()  # restart timer
            return
    # print("red - 1 deg")
    dxmit('1')
    butnred2 = False			# double tap started
    lastpress = RED
    redepoch = time.time()  	# start timer
...
def Button_black():
    ...
    if lastpress == BLACK:
        now = time.time()
        delta_t = now - blackepoch
        # print("now", now)
        # print("delta_t", delta_t)
        if (delta_t < bb_delay): # if less than .6 sec
            # print("all stop")
            dxmit('0')
            lastpress = None
            return
    ...
    dxmit('5')
    lastpress = BLACK
    blackepoch = time.time()    # start timer
    # print("now", blackepoch)

def Button_yellow():			# reduce speed 10%
    global lastpress
    # print('yellow')
    dxmit('8')
    lastpress = YELLOW

def Button_blue_released():				# increase speed 10%
    global lastpress
    # print("blue released")
    dxmit('2')
    lastpress = BLUE
    
def Button_blue():				# increase speed 10%
    global lastpress
    # print("blue")
    dxmit('2')
    lastpress = BLUE
...
def Chart():
    ...
    points = Utm2Pix(proppts, scale, stlat, stlon)
    ...
    house = Utm2Pix(housepts, scale, stlat, stlon)
    ...
    '''
    h3 = Utm2Pix(tuffshed, scale, stlat, stlon)
...
#     canvas.create_polygon(work, outline='black', fill='red', width=1, tags='bldg')
    tgar = Utm2Pix(trailergar, scale, stlat, stlon)
    canvas.create_polygon(tgar, outline='black', fill='gray50', width=1, tags='bldg')
    shed = Utm2Pix(shedrow, scale, stlat, stlon)
    canvas.create_polygon(shed, outline='black', fill='gray50', width=1, tags='bldg')
    back = Utm2Pix(backpts, scale, stlat, stlon)
    bdrv = canvas.create_polygon(back, outline='black', fill='gray75', width=1, tags="pave")
    frtdr = Utm2Pix(frtdrv, scale, stlat, stlon)
    fdrv = canvas.create_polygon(frtdr, outline='black', fill='gray75', width=1, tags="pave")
    front = Utm2Pix(frontpts, scale, stlat, stlon)
    fdrv = canvas.create_polygon(front, outline='black', fill='gray75', width=1, tags="pave")
    conc = Utm2Pix(apron, scale, stlat, stlon)
    aprn = canvas.create_polygon(conc, outline='black', fill='gray75', width=1, tags="pave")
    concE = Utm2Pix(apron2, scale, stlat, stlon)
    aprn2 = canvas.create_polygon(concE, outline='black', fill='gray75', width=1, tags="pave")
    concN = Utm2Pix(apronpath, scale, stlat, stlon)
    aprnN = canvas.create_polygon(concN, outline='black', fill='gray75', width=1, tags="pave")
#    trees = Usf2Pix(alltrees, scale, stlat, stlon)
    trees = Utm2Pix(alltrees, scale, stlat, stlon)
    llen = len(trees)
    rad = scale
    print(trees[0],trees[1])
...
#    wpts = Usf2Pix(waypts, scale, stlat, stlon)
    wpts = Utm2Pix(waypts, scale, stlat, stlon)
    llen = len(wpts)
    print(wpts[0],wpts[1])
    for i in range(0, llen, 2):
#        canvas.create_rectangle(wpts[i]-rad, wpts[i+1]-rad, wpts[i]+1+rad, wpts[i+1]+1+rad, fill='deep sky blue', outline='blue', tags = "wpts")
        canvas.create_rectangle(wpts[i], wpts[i+1], wpts[i]+4, wpts[i+1]+4, fill='deep sky blue', outline='blue', tags = "wpts")
...
def Xspot(xlon, xlat):
    spot = Mtrs2Pix([[xlon, xlat]], scale, stlat, stlon)
...
# convert array of Z11N pairs into linear array of pixels (in pairs)
# slat, slon starting lat/lon usf from 34-14, -119-04
def Utm2Pix(utm, gscale, slat, slon):
    pix = []
    for x in utm:
        pix.append(int(gscale * (slon+x[0] - biasZ11x)))
        pix.append(int(gscale * (slat-(x[1] - biasZ11y))))
    return pix

# convert relative meter coord into pixel coord
def Mtrs2Pix(utm, gscale, slat, slon):
    pix = []
    for x in utm:
        pix.append(int(gscale * (slon+x[0])))
        pix.append(int(gscale * (slat-x[1])))
...
# 0:move, 1:zoom in, 2:zoom out, 3:goto
def Bpress(event):
    global mx
    global my
    mx = event.x
...
#        print (str(lat)+"/"+str(lon))
        ...
        # check lat/lon vs Z11N east/north : lat/lon
        msg = '{GN%7.2f}' % (biasZ11x + plon)
        ser.write(msg.encode('utf-8'))
        print(msg)
        msg = '{GT%7.2f}' % (plat + biasZ11y)
        ser.write(msg.encode('utf-8'))
        print(msg)
        ...
        lltree = alltrees[0]
        i = 0
        treename.set("No Tree")
        treetype.set("")
        for x in alltrees:
            dellon = abs(x[0] - (plon + biasZ11x))
            dellat = abs(x[1] - (plat + biasZ11y))
            if i == 1:
                print(plon, plat)
                print(x[0], x[1])
            if (dellat < 1.0 and dellon < 1.0):
                treename.set(tnames[i])
                treetype.set(ttypes[i])
                break
            i = i + 1
                ...
                if not butngreen:                      # if not stale tap
                    if ((time.time() - greenepoch) < .6): # if less than .6 sec
                        if butngreen2:                 # if double tap in progress
                            ...
                            dxmit('6')            # 5 deg steering
                            butngreen2 = True          # double tap started
                    else:                              # else 1st tap
                        dxmit('3')                # 1 deg steering
            ...
            if (GPIO.input(13) == False):
                if not butnred:
                    if ((time.time() - redepoch) < .6): # if less than .6 sec
                        if butnred2:
                            dxmit('7')             # 35 deg steering
                        else:
                            dxmit('4')             # 5 deg steering
                            butnred2 = True
                    else:
                        dxmit('1')                 # 1 deg steering
                        butnred2 = False
                    butnred = True
                    redepoch = time.time()
            else:
...
rc = cdlib.CDbLogin("robodatabaseM2606a.dbf".encode())
#rc = cdlib.CDbLogin("robodatabase.dbf".encode())
#rc = cdlib.Cdblogin()
print("dbopen ", rc)
...
ttypes = []
while (rc >= 0):
    lon = cdlib.CGetDouble(treetable, "MetEast".encode())
    lat = cdlib.CGetDouble(treetable, "MetNorth".encode())
#    lon = lon - biasZ11x
#    lat = lat - biasZ11y
    alltrees.append([lon, lat])
    pstr = cdlib.CGetCharPtr(treetable, "Name".encode())
    tnames.append(pstr.decode())
    pstr = cdlib.CGetCharPtr(treetable, "Variety".encode())
    ttypes.append(pstr.decode())
    rc = cdlib.CNext(treetable, locndx)
 
print(tnames[1], ttypes[1])
...
if BUTTON == 0:
    buttongreen.when_pressed = Button_green
    buttonred.when_pressed = Button_red
    buttonblack.when_pressed = Button_black
    # buttonblue.when_released = Button_blue_released
    buttonblue.when_pressed = Button_blue
    buttonyellow.when_pressed = Button_yellow
