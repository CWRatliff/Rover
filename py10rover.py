...
def simple_commands(schr):
    ...
    #             max_turn(Left_Limit, speed)
        ...
        case '9':                   # 9 - GEE steer right limit
            if (auto):
                if (wptflag):
...
#============================ pan/tilt camera
        case 'Q':           #dont forget to change in tframe
            pan += 5
    ...
    #        pass
        case 'D':
            for ang in range(140, 0, -1):
                ...
                time.sleep(0.1)
    #        pass
        case 'I':           # switch off: I, J, K
            robot.switch(0, 0)
        case 'X':            # switch on: X, Y, Z
            robot.switch(0, 1)
...
def commander(cmdbuff):
    ...
    xcmd = cmdbuff[1]
    match xcmd:
        case 'D':       			# single digit keypad commands
            xcmd = cmdbuff[2]
            simple_commands(xcmd)
            cogBase = 0         	#invalidate COG baseline

        case 'E':       			# Keypad commands preceded by a star
            xcmd = cmdbuff[2]
            star_commands(xcmd)
            cogBase = 0         	#invalidate COG baseline

        case 'F':       			# Keypad commands preceeded by a #
            route_waypoint(cmdbuff)

        case "G":       			# goto lat/lon command
            goto_command(cmdbuff)

        case 'L':       			#lat/long input from GPS h/w
            GPS_data(cmdbuff)

        case 'O':       			#O - orientation esp hdg from arduino
            yaw = int(cmdbuff[2:msglen-1])
            hdg = (yaw + compass_bias)%360

        case 'P':       			#P - gcode script
...
#           print("heartbeat")
        ...
        case 'T':       			#'D' key + number button Diagnostic
            xcmd = cmdbuff[2]
            diag_commands(xcmd)
